...
public class Question03 {
	...
	public static void main(String[] args) {
		...
		while(control < q) {
			a[control] = sc.nextInt();
			b[control] = sc.nextInt();
			n[control] = sc.nextInt();
			control++;
		}
		sc.close();
		
		for(int i = 0 ; i < q ; i++) {
			for(int j = 0,s=0, x=1; j < n[i] ; j++) {
				if(j==0) {
					s = s + a[i] + (x*b[i] );
					x++;
				}else {
...
print the series corresponding to the given a, b, and n values 
...
8 14 26 50 98
   ...
   Explanation
We have two queries:
1.We use a = 0, b=2 and n = 10 to produce some series s0,s1,...
sn-1:
-s0 = 0 + 1*2 = 2
-s1 = 0 + 1*2 + 2*2 = 6
-s2 = 0 + 1*2 + 2*2 + 4*2 = 14
... and so on.
Once we hit n=10, we print the first ten terms as a single line 
of space-separated integers.

2.We use a = 5, b=3 and n = 5 to produce some series s0,s1,...
sn-1:
-s0 = 5 + 1*3 = 8
-s1 = 5 + 1*3 + 2*3 = 14
-s2 = 5 + 1*3 + 2*3 + 4*3 = 26
