package Beecrowd;

import java.util.Scanner;

public class Ex10 {
    public static void main(String[] args) {

        Scanner sc = new Scanner(System.in);
        int codPeca = sc.nextInt() , nPecas = sc.nextInt(); Double vPeca = sc.nextDouble();
        int codPeca2 = sc.nextInt() , nPecas2 = sc.nextInt(); Double vPeca2 = sc.nextDouble();
        sc.close();
        
        double vTotal = (nPecas * vPeca) + (nPecas2 * vPeca2);

        if (codPeca<=10) {
            System.out.println("Cosmeticos");
        } else if (codPeca > 10 && codPeca <= 25) {
            System.out.println("Alimentos ");
        }else if (codPeca > 25){
            System.out.println("Ferramentas");
        }

        if (codPeca2<=10) {
            System.out.println("Cosmeticos");
        } else if (codPeca2 > 10 && codPeca2 <= 25) {
            System.out.println("Alimentos");
        }else if (codPeca2 > 25){
            System.out.println("Ferramentas ");
        }

        System.out.printf("VALOR A PAGAR: R$ %.2f %n" , vTotal);
    }
}

/*
Neste problema, deve-se ler o código de uma peça 1, o número de peças 1, o valor unitário de cada peça
 1, o código de uma peça 2, o número de peças 2 e o valor unitário de cada peça 2. Após, calcule 
 e mostre o valor a ser pago.

Entrada
O arquivo de entrada contém duas linhas de dados. Em cada linha haverá 3 valores, respectivamente 
dois inteiros e um valor com 2 casas decimais.

Saída
A saída deverá ser uma mensagem conforme o exemplo fornecido abaixo, lembrando de deixar um espaço
 após os dois pontos e um espaço após o "R$". O valor deverá ser apresentado com 2 casas após o ponto. 
*/